...
import DESIGNS from "./notebookDesigns";

// Opens the writable notebook as a standalone popup -- matching the
// FORGE/ASCEND/Article Player chrome-less window.open pattern. The hub
// itself stays a normal in-app page; only the writing surface pops out.
function openNotebook(designKey) {
  const screenW = window.screen.availWidth || 1400;
  const screenH = window.screen.availHeight || 900;
  const w = Math.min(760, screenW - 40);
  const h = Math.min(920, screenH - 80);
  const left = Math.max(0, Math.floor((screenW - w) / 2));
  const top = Math.max(0, Math.floor((screenH - h) / 2));

  window.open(
    `/library/notebook/${designKey}`,
    "sentivoNotebook",
...
.nbh-eyebrow {
  ...
  color: #1E8F76;
  background: rgba(63,205,175,0.14);
  border-radius: 999px;
  padding: 5px 14px;
  margin-bottom: 14px;
}
...
.nbh-card {
  ...
  cursor: pointer;
  padding: 0;
  box-shadow: 0 10px 24px rgba(30,143,118,0.08);
  transition: transform 0.15s ease, box-shadow 0.15s ease;
}
.nbh-card:hover { transform: translateY(-3px); box-shadow: 0 16px 32px rgba(30,143,118,0.15); }
...
.nbh-card-blurb { font-size: 12.5px; line-height: 1.5; color: #5A6E68; margin: 0; }
.nbh-card-cta {
  margin-top: 8px;
  font-size: 11.5px;
  font-weight: 700;
...
@media (max-width: 600px) {
  ...
  .nbh-stage { padding: 32px 18px 48px; }
}
`;
